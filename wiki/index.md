# NFs Research Wiki

This wiki is the synthesis layer for the FlashSim normalizing-flow research project. Source code, configurations, manifests, ROOT files, and experiment outputs remain the source of truth.

## Project

- [[Project Overview]]
- [[Current Research Status]]
- [[End-to-End Workflow]]
- [[NFs Pipeline Architecture]]
- [[Model 4 Training Vertical Slice]]
- [[Training Resolution and Preflight]]
- [[Source and Provenance]]

## Models

- [[Model 1]]
- [[Model 2]]
- [[Model 3]]
- [[Model 4]]

## Core theory and physics

- [[Normalizing Flows]]
- [[Maximum Likelihood]]
- [[Rational-Quadratic Spline]]
- [[Weighted Density]]
- [[FLUKA Weight]]

## Data and preprocessing

- [[Dataset Registry]]
- [[Preprocessing Pipelines]]
- [[Preprocessing A]]
- [[Preprocessing B]]
- [[Preprocessing C]]

## Validation and experiments

- [[Validation Strategy]]
- [[CCDF]]
- [[C2ST]]
- [[Correlation Validation]]
- [[Tail Validation]]
- [[Model 4 Selection 2022-2025]]
- [[Epoch 450 Branched Resume]]
- [[Controlled Hyperparameter Experiments]]
- [[Train versus All-FLUKA Guard]]

## Decisions and open work

- [[Decision Index]]
- [[Paths and Seeds]]
- [[Open Questions]]
- [[log|Research Log]]
- [[Daily Work Log]]
